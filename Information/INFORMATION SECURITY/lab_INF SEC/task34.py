...
def linear_cryptoanalysis_KPA(u,x):
    ...
    #does not work this automatic mod function
    #k = np.mod(k,2)
    #print(k)
...
'''
def turn_NP_array(ls):
    myNpArray = np.zeros([0, len(ls)])
    for i in range(0,len(ls),1):
        row = ls[i]
        myNpArray = np.vstack((myNpArray, row))

    myNpArray = myNpArray[0:]
